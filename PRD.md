...
**Status:** Live in production (Vercel + Supabase). This document reflects
...
### 4.4 Plan Limits
- Screens per org capped by `organisations.plan` (`free` = 1, enforced at
  screen-creation time in the API route)
...
- Stripe Checkout (hosted) for subscriptions; no direct card handling
...
- **Not yet built**: Stripe integration, email provider (Resend planned),
  scheduled lifecycle job, and the schema additions needed for all of the
  above
...
- No video transcoding — uploads must already be browser-playable (H.264
  MP4 recommended)
- No plan-upgrade/billing UI yet (tracked in BILLING_PRD.md)
